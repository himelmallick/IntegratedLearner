...
test_that("IntegratedLearner runs on an iHMP subset with validation set", {
  ...
  keep_samples <- sort(unique(unlist(lapply(class_map, function(ids) {
    ids[seq_len(min(
      10,
      length(ids)
    ))]
  ...
  keep_features <- sort(unique(unlist(lapply(layers, function(layer) {
    head(rownames(pcl$feature_metadata)[pcl$feature_metadata$featureType ==
      layer], 12)
  }), use.names = FALSE)))

  pcl_small <- subset_pcl(pcl, feature_ids = keep_features, sample_ids = keep_samples)
  pcl_small$sample_metadata <- pcl_small$sample_metadata[, c("Y", "subjectID"),
    drop = FALSE
  ]
  pcl_small$feature_metadata <- pcl_small$feature_metadata[, c("featureID", "featureType"),
    drop = FALSE
  ]

  class_map_small <- split(rownames(pcl_small$sample_metadata), as.character(pcl_small$sample_metadata$Y))
  train_ids <- sort(unique(unlist(lapply(class_map_small, function(ids) {
