make_toy_multiclass_pcl <- function(
  ...
  feature_metadata <- data.frame(featureID = feature_ids, featureType = rep(c(
    "omicsA",
    "omicsB"
  ), length.out = n_features), row.names = feature_ids, stringsAsFactors = FALSE)
...
test_that("IntegratedLearner multiclass path runs with native multiclass backend", {
  ...
  fit <- suppressWarnings(IntegratedLearner::IntegratedLearner(
    PCL_train = pcl,
    folds = 2, seed = 2026, base_learner = "SL.glmnet", run_stacked = TRUE,
    run_concat = TRUE, print_learner = FALSE, family = stats::binomial()
  ))
  ...
  expect_true(all(c("model", "accuracy", "balanced_accuracy", "logloss") %in%
    colnames(fit$metrics.train)))

  p <- fit$prob.train[["stacked"]]
  expect_true(is.matrix(p) || is.data.frame(p))
  expect_equal(nrow(p), nrow(pcl$sample_metadata))
  expect_true(all(abs(rowSums(as.matrix(p)) - 1) < 1e-06))
})
...
test_that("predict.learner returns multiclass probabilities and metrics", {
  ...
  fit <- suppressWarnings(IntegratedLearner::IntegratedLearner(
    PCL_train = pcl,
    folds = 2, seed = 2027, base_learner = "SL.glmnet", run_stacked = TRUE, run_concat = TRUE,
    print_learner = FALSE, family = stats::binomial()
  ))

  pred <- IntegratedLearner:::predict.learner(
    object = fit, feature_table_valid = pcl$feature_table,
    sample_metadata_valid = pcl$sample_metadata, feature_metadata = pcl$feature_metadata
  )
...
test_that("update.learner is blocked with explicit message for multiclass fits", {
  ...
  fit <- suppressWarnings(IntegratedLearner::IntegratedLearner(
    PCL_train = pcl,
    folds = 2, seed = 2028, base_learner = "SL.glmnet", run_stacked = TRUE,
    run_concat = FALSE, print_learner = FALSE, family = stats::binomial()
  ))

  expect_error(
    IntegratedLearner:::update.learner(
      object = fit, feature_table_valid = pcl$feature_table,
      sample_metadata_valid = pcl$sample_metadata, feature_metadata_valid = pcl$feature_metadata
    ),
...
test_that("IntegratedLearner multiclass supports xgboost backend", {
  ...
  fit <- suppressWarnings(IntegratedLearner::IntegratedLearner(
    PCL_train = pcl,
    folds = 2, seed = 2029, base_learner = "xgboost", meta_learner = "xgboost",
    run_stacked = TRUE, run_concat = FALSE, print_learner = FALSE, family = stats::binomial()
  ))
...
test_that("IntegratedLearner multiclass supports randomForest backend", {
  ...
  fit <- suppressWarnings(IntegratedLearner::IntegratedLearner(
    PCL_train = pcl,
    folds = 2, seed = 2031, base_learner = "randomforest", meta_learner = "randomforest",
    run_stacked = TRUE, run_concat = FALSE, print_learner = FALSE, family = stats::binomial()
  ))
...
test_that("IntegratedLearner multiclass supports mbart backend", {
  ...
  fit <- suppressWarnings(IntegratedLearner::IntegratedLearner(
    PCL_train = pcl,
    folds = 2, seed = 2033, base_learner = "mbart", meta_learner = "glmnet",
    run_stacked = FALSE, run_concat = FALSE, print_learner = FALSE, family = stats::binomial(),
    ntree = 10, ndpost = 20, nskip = 10, keepevery = 1
  ))
...
test_that("mbart prediction aligns columns to training features", {
  ...
  y <- factor(rep(c("A", "B", "C"), length.out = nrow(X)), levels = c(
    "A", "B",
    "C"
  ))
  ...
  fit_obj <- suppressWarnings(IntegratedLearner:::.fit_multiclass_model_impl(
    ...
    y = y, learner_id = "mbart", seed = 2040, model_args = list(
      ntree = 10L,
      ndpost = 20L, nskip = 10L, keepevery = 1L, printevery = 1000L
    )
  ))
...
test_that("credint.learner supports multiclass mbart fits", {
  ...
  fit <- suppressWarnings(IntegratedLearner::IntegratedLearner(
    PCL_train = pcl,
    folds = 2, seed = 2042, base_learner = "mbart", meta_learner = "glmnet",
    run_stacked = FALSE, run_concat = FALSE, print_learner = FALSE, family = stats::binomial(),
    ntree = 10, ndpost = 20, nskip = 10, keepevery = 1
  ))
...
test_that("credint.learner supports multiclass mbart concatenated uncertainty", {
  ...
  fit <- suppressWarnings(IntegratedLearner::IntegratedLearner(
    PCL_train = pcl,
    folds = 2, seed = 2043, base_learner = "mbart", meta_learner = "glmnet",
    run_stacked = FALSE, run_concat = TRUE, print_learner = FALSE, family = stats::binomial(),
    ntree = 10, ndpost = 20, nskip = 10, keepevery = 1
  ))
  ...
  p <- suppressWarnings(IntegratedLearner:::credint.learner(
    fit = fit_bad_names,
    test = FALSE, model = "concatenated", class = "C1"
  ))
...
test_that("multiclass screening keeps top X% features", {
  ...
  fit <- suppressWarnings(IntegratedLearner::IntegratedLearner(
    PCL_train = pcl,
    folds = 3, seed = 2044, base_learner = "glmnet", meta_learner = "glmnet",
    run_screening = TRUE, screen_pct = 25, run_stacked = FALSE, run_concat = TRUE,
    print_learner = FALSE, family = stats::binomial()
  ))
...
test_that("multiclass prevalence_pct keeps top X% features by prevalence", {
  ...
  fit <- suppressWarnings(IntegratedLearner::IntegratedLearner(
    PCL_train = pcl,
    folds = 3, seed = 2045, base_learner = "glmnet", meta_learner = "glmnet",
    run_stacked = FALSE, run_concat = TRUE, print_learner = FALSE, family = stats::binomial(),
    prevalence_pct = 20
  ))
  ...
  layer_sizes <- table(as.character(pcl$feature_metadata$featureType))
  expected_kept <- as.integer(sum(ceiling(0.2 * as.numeric(layer_sizes))))
  expect_equal(length(fit$feature.names), expected_kept)
})
...
test_that("multiclass filter_method='prevalence' with filter_pct keeps top X% features", {
  ...
  fit <- suppressWarnings(IntegratedLearner::IntegratedLearner(
    PCL_train = pcl,
    folds = 3, seed = 2046, base_learner = "glmnet", meta_learner = "glmnet",
    run_stacked = FALSE, run_concat = TRUE, print_learner = FALSE, family = stats::binomial(),
    filter_method = "prevalence", filter_pct = 20
  ))
...
test_that("multiclass filter_method='variance' keeps top X% features by caret variance ranking", {
  ...
  fit <- suppressWarnings(IntegratedLearner::IntegratedLearner(
    PCL_train = pcl,
    folds = 3, seed = 2047, base_learner = "glmnet", meta_learner = "glmnet",
    run_stacked = FALSE, run_concat = TRUE, print_learner = FALSE, family = stats::binomial(),
    filter_method = "variance", filter_pct = 20
  ))
