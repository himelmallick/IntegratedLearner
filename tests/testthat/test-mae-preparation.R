test_that("infer_mae_defaults selects experiments and assays", {
  mae <- make_toy_mae(
    n_samples = 12, n_features_layer1 = 6, n_features_layer2 = 4,
    seed = 31
  )
...
test_that("infer_mae_defaults validates experiment and assay arguments", {
  mae <- make_toy_mae(
    n_samples = 10, n_features_layer1 = 5, n_features_layer2 = 5,
    seed = 32
  )
...
test_that("get_data_from_MAE handles survival fallback and required colData columns", {
  mae <- make_toy_mae(
    ...
    seed = 34, outcome = "survival", include_time_event = TRUE
  )
  ...
  expect_true(all(c(
    "sampleID", "subjectID", "Y", "time", "event", "featureID",
    "experiment"
  ) %in% colnames(long_df)))
  expect_true(nrow(long_df) > 0)

  mae_missing_subject <- make_toy_mae(
    n_samples = 10, n_features_layer1 = 5,
    n_features_layer2 = 4, seed = 35
  )
...
test_that("prepare_from_MAE returns canonical objects and validates feature identity", {
  mae <- make_toy_mae(
    n_samples = 14, n_features_layer1 = 6, n_features_layer2 = 5,
    seed = 36
  )
