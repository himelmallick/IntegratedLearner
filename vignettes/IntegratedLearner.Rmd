...
| `intermediate_learners` | `NULL` (non-survival), `c("surv.coxph")` (survival default) | all | Optional intermediate-fusion learners. Continuous/binary currently support `"multiview"`. Survival additionally supports `"surv.multiview"`. Multiclass intermediate fusion is not yet implemented. |
| `multiview_rho_grid` | `c(0, 0.1, 0.25, 0.5, 1)` | continuous, binary, survival | Cooperative-learning `rho` grid used when `multiview`-based intermediate fusion is enabled. |
| `multiview_s` | `"lambda.min"` | continuous, binary, survival | Lambda rule used when extracting `multiview` predictions (`"lambda.min"` or `"lambda.1se"`). |
| `multiview_alpha` | `1` | continuous, binary, survival | Elastic-net `alpha` passed to `multiview`. |
...
| `intermediate_learners` | `NULL` | Optional intermediate learners; currently supports `"multiview"` via the CRAN `multiview` package. |
| `multiview_rho_grid`, `multiview_s`, `multiview_alpha` | defaults above | Cooperative-learning tuning controls used when `"multiview"` is enabled. |
...
| `intermediate_learners` | ignored | Reserved for future multiclass intermediate fusion; currently ignored with a message. |
...
| `intermediate_learners` | `c("surv.coxph")` | Intermediate fusion learners. Supports risk-level learners such as `"surv.coxph"` and cooperative feature-level fusion via `"surv.multiview"`. |
| `multiview_rho_grid`, `multiview_s`, `multiview_alpha` | defaults above | Cooperative-learning tuning controls used when `"surv.multiview"` is enabled. |
...
|---|---|---|---|---|
| Continuous/Binary | Yes | `run_concat = TRUE` | `run_stacked = TRUE` with `meta_learner` | `intermediate_learners = "multiview"` |
| Multiclass | Yes | `run_concat = TRUE` | `run_stacked = TRUE` with native multiclass `meta_learner` | Not yet implemented |
| Survival | Yes (`train_out$single`) | `do_early_fusion = TRUE` | Weighted layer blending (`weight_method = "IBS"` or `"COX"`) | `intermediate_learners`, including `surv.multiview` |
...
| Intermediate fusion | Cooperative learner over per-layer feature matrices | Enable with `intermediate_learners = "multiview"`; outputs in `fit$yhat.train[, "intermediate_multiview"]`, `fit$model_fits$model_intermediate$multiview`, `fit$intermediate_details$multiview` |
