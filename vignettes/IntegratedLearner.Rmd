...
load_extdata_rdata <- function(filename, envir = parent.frame()) {
  path <- system.file("extdata", filename, package = "IntegratedLearner")
  if (!nzchar(path)) {
    stop("Could not find extdata file: ", filename)
  }
...
```{r}
# Training data
load_extdata_rdata("PRISM.RData", envir = environment())
...
load_extdata_rdata("NLIBD.RData", envir = environment())
...
```{r}
load_extdata_rdata("pregnancy.RData", envir = environment())
...
```{r}
load_extdata_rdata("FranzosaE_2019.RData", envir = environment())
load_extdata_rdata("FranzosaE_2019_Validation.RData", envir = environment())
...
```{r}
load_extdata_rdata("TCGA_BRCA.RData", envir = environment())
