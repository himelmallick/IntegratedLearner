...
`IntegratedLearner` is designed for multi-omics studies where each assay layer can carry complementary signal and where users want a unified interface for per-layer modeling, early fusion, late fusion, and direct cooperative multiview learning. This makes the package a natural fit for Bioconductor workflows built around `MultiAssayExperiment`, `SummarizedExperiment`, and curated assay containers.
...
- Intermediate/cooperative fusion: direct `multiview` cooperative learning over feature matrices from each layer.

Cooperative learning is enabled with `run_intermediate = TRUE`. It is currently supported for continuous, binary, and survival outcomes. For multiclass outcomes, `IntegratedLearner` prints a message and ignores `run_intermediate`.
...
| `run_intermediate` | `FALSE` | continuous, binary, survival | Enables direct cooperative learning through `multiview`. Multiclass fits print a message and ignore this option. Requires at least 3 CV folds. |
...
|---|---|---|---|---|
| Continuous/Binary | Yes | `run_concat = TRUE` | `run_stacked = TRUE` with `meta_learner` | `run_intermediate = TRUE` via `multiview` |
...
| Survival | Yes (`train_out$single`) | `do_early_fusion = TRUE` | Weighted layer blending with both `IBS` and `COX` outputs returned | `run_intermediate = TRUE` via `multiview` Cox model |
...
| Cooperative/intermediate fusion | Direct cooperative `multiview` model trained on layer feature matrices | Enable with `run_intermediate = TRUE`; outputs in `fit$yhat.train[, "cooperative"]`, `fit$yhat.test[, "cooperative"]`, `fit$model_fits$model_cooperative`, `fit$cooperative_rho`, and `fit$cooperative_rho_scores` |
...
| Cooperative/intermediate fusion | `fit$train_out$cooperative`, containing the selected `multiview` model, C-index, time-dependent AUC, and risk scores | `fit$valid_out$cooperative`, with analogous validation metrics and risk scores |
...
To add direct cooperative survival modeling, use at least 3 folds and set the cooperative options:
