...
# Tutorial dependencies used in evaluated chunks
library(dplyr)
library(ggplot2)
...
For this build-friendly example, we use a stable baseline learner (`SL.glm`) with 5-fold CV.
An optional BART-only interpretation block is included below and runs only when `fit$base_learner = "SL.BART"` and `bartMachine` is available.
...
is_bart_fit <- identical(fit$base_learner, "SL.BART") &&
  requireNamespace("bartMachine", quietly = TRUE)

if (!isTRUE(is_bart_fit)) {
  message("Skipping BART-specific posterior and feature-importance chunks in this run.")
}
```
...
### Step 5: Uncertainty and Feature-Level Interpretation (BART, optional)

When using `SL.BART`, you can inspect posterior predictive distributions and derive weighted posterior summaries.

```{r, eval=exists("is_bart_fit") && isTRUE(is_bart_fit)}
weights <- fit$weights
...
```{r fig.width=9, fig.height=7, eval=exists("is_bart_fit") && isTRUE(is_bart_fit)}
ord_names <- names(sort(rowMeans(weighted.post.samples), decreasing = TRUE))

bayesplot::mcmc_intervals(t(weighted.post.samples), prob = 0.68, prob_outer = 0.95) +
...
```{r fig.width=7, fig.height=4, eval=exists("is_bart_fit") && isTRUE(is_bart_fit)}
cowplot::plot_grid(
  p4,
  ncol = 1,
...
fit_surv_mae <- IntegratedLearner(
  ...
  assay.type = c("relative_abundance", "pathway_abundance"),
  folds = 2,
  base_learner = "surv.coxboost",  # alternatives include surv.coxph/surv.ranger/surv.xgboost.cox/...
