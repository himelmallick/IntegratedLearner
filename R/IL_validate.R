.validate_IL_inputs <- function(
  ...
  ## ---------- Required columns ----------
  required_sample_cols <- if (is_survival) {
    c("time", "event", "subjectID")
  } else {
    c("Y", "subjectID")
  }
  if (!all(required_sample_cols %in% colnames(sample_metadata))) {
    stop("sample_metadata must contain columns: ", paste(required_sample_cols,
      collapse = ", "
    ))
  }
  ...
  if (!all(required_feature_cols %in% colnames(feature_metadata))) {
    stop("feature_metadata must contain columns: ", paste(required_feature_cols,
      collapse = ", "
    ))
  }
  ...
  if (!is_survival) {
    ...
    if (identical(family_name, "binomial")) {
      ...
      # Backwards-compatible warning for binary settings not coded as
      # 0/1.
      if (length(unique(y_chr)) == 2L) {
        y_num_parsed <- type.convert(as.character(Y), as.is = TRUE)
        y_num <- if (is.numeric(y_num_parsed)) {
          as.numeric(y_num_parsed)
        } else {
    ...
    if (any(!is.finite(tvec))) {
      stop("Survival time contains non-finite values.")
    }
    if (any(!is.finite(evec))) {
      stop("Survival event indicator contains non-finite values.")
    }
    if (any(tvec < 0)) {
      stop("Survival time must be non-negative.")
    }
    if (!all(evec %in% c(0, 1))) {
      stop("Survival event indicator must be in {0,1}.")
    }
  }
  ...
  if (!is.null(feature_table_valid)) {
    ...
    if (is_survival && !is.null(sample_metadata_valid)) {
      ...
      if (any(!is.finite(tv))) {
        stop("Validation survival time contains non-finite values.")
      }
      if (any(!is.finite(ev))) {
        stop("Validation survival event indicator contains non-finite values.")
      }
      if (any(tv < 0)) {
        stop("Validation survival time must be non-negative.")
      }
      if (!all(ev %in% c(0, 1))) {
        stop("Validation survival event indicator must be in {0,1}.")
      }
    }
  }
...
.validate_multiclass_training_inputs <- function(sample_metadata, folds) {
  ...
  if (!("subjectID" %in% colnames(sample_metadata))) {
    stop("sample_metadata must contain column 'subjectID' for multiclass cross-validation.",
      call. = FALSE
    )
  }
  ...
  if (length(subjectID) < folds) {
    ...
    warning("Reduced folds to the number of unique subject IDs for multiclass training.",
      call. = FALSE
    )
  }

  list(
    Y = factor(y_raw, levels = class_levels), class_levels = class_levels, subjectID = subjectID,
    folds = folds
  )
}
...
.validate_multiclass_prediction_inputs <- function(fit, feature_table_valid, feature_metadata) {
  ...
  if (!"featureID" %in% colnames(feature_metadata) || !"featureType" %in% colnames(feature_metadata)) {
    stop("feature_metadata must contain both 'featureID' and 'featureType'.",
      call. = FALSE
    )
  }
...
.validate_survival_core_inputs <- function(
  ...
  if (!(base_learner %in% supported_learners)) {
    stop("Unsupported base_learner. Supported: ", paste(supported_learners, collapse = ", "),
      call. = FALSE
    )
  }
  ...
  list(feature_metadata = feature_metadata, model_args = model_args, layers = layers)
}
