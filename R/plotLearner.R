...
plot.learner <- function(
  ...
  if (fit$family == "binomial") {
    ...
    for (k in seq_along(list.ROC)) {
      ...
      list.ROC[[k]] <- data.frame(
        ...
        specificity = 1 - methods::slot(perf, "x.values")[[1]], AUC = AUC,
        layer = names(list.ROC)[k], method = method
      )
    }
    ...
    p1 <- ggplot2::ggplot(plot_data, ggplot2::aes(
      ...
      ggplot2::theme(
        ...
        legend.background = ggplot2::element_blank(), legend.box.background = ggplot2::element_rect(colour = "black")
      ) +
      ggplot2::theme_bw() +
    ...
    if (isTRUE(fit$test)) {
      ...
      for (k in seq_along(list.ROC.valid)) {
        ...
        list.ROC.valid[[k]] <- data.frame(
          sensitivity = methods::slot(
            perf,
            "y.values"
          )[[1]], specificity = 1 - methods::slot(perf, "x.values")[[1]],
          AUC = AUC, layer = names(list.ROC.valid)[k], method = method
        )
      }

      ROC_table_valid <- do.call(rbind, list.ROC.valid)
      plot_data2 <- ROC_table_valid
      plot_data2$displayItem <- paste(plot_data2$layer, " AUC = ", plot_data2$AUC,
        sep = ""
      )
      plot_data2$displayItem <- factor(plot_data2$displayItem, levels = unique(plot_data2$displayItem))

      p2 <- ggplot2::ggplot(plot_data2, ggplot2::aes(
        x = specificity, y = sensitivity,
        group = displayItem
      )) +
        ggplot2::geom_line(ggplot2::aes(color = displayItem)) +
      ...
      p <- cowplot::plot_grid(p1, p2,
        nrow = 2, labels = c(paste0(
          "A. ", fit$folds,
          "-fold CV"
        ), "B. Independent Validation"), label_size = label_size,
        label_x = label_x, vjust = vjust
      ) + ggplot2::theme(plot.margin = grid::unit(c(
        1,
        1, 1, 1
      ), "cm"))
    ...
    p <- cowplot::plot_grid(p1,
      nrow = nrow_plot, ncol = ncol_plot, labels = c(paste0(
        "A. ",
        fit$folds, "-fold CV"
      )), label_size = label_size, label_x = label_x,
      vjust = vjust
  ...
  if (fit$family == "gaussian") {
    ...
    p <- cowplot::plot_grid(p1, ncol = 1, labels = c(paste0(
      "A. ", fit$folds,
      "-fold CV"
  ...
  if (fit$family == "multinomial") {
    ...
    if (exists("test_tbl")) {
      out$metrics_test <- test_tbl
    }
    return(out)
  }

  stop("Unknown family in fit$family. Expected 'binomial', 'gaussian', or 'multinomial'.",
    call. = FALSE
  )
}
