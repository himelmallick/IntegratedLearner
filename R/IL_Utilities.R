...
#'   distribution. For example, when k=2, the prior probability is 95 percent. For
...
SL.BART <- function(
  ...
  model <- tryCatch(bartMachine::bartMachine(X, Y,
    num_trees = num_trees, num_burn_in = num_burn_in,
    verbose = verbose, alpha = alpha, beta = beta, k = k, q = q, nu = nu, num_iterations_after_burn_in = num_iterations_after_burn_in,
    serialize = serialize, seed = seed
  ), error = function(e) {
    msg <- conditionMessage(e)
    if (grepl("NoClassDefFoundError", msg) && grepl("jdk/incubator/vector/Vector",
      msg,
      fixed = TRUE
    )) {
...
########################################## Elastic net wrapper with a fixed
########################################## alpha #
...
SL.glmnet2 <- function(
  ...
  # string.
  pred <- stats::predict(fitCV, newx = newX, type = "response", s = ifelse(useMin,
...
############################################ Elastic net wrapper with optimized
############################################ alpha #
...
.infer_mae_defaults <- function(
  ...
  if (is.null(experiment)) {
    ...
    if (verbose) {
      message("IntegratedLearner: using all experiments in mae_train: ", paste(experiment,
        collapse = ", "
      ))
    }
  } else {
...
.get_data_from_MAE <- function(
  ...
  for (i in seq_along(experiment)) {
    ...
    if (!assay_name %in% SummarizedExperiment::assayNames(se)) {
      stop(
        "Assay '", assay_name, "' not found in experiment '", exp_name,
        "'."
      )
    }
    ...
    if (nrow(mat) == 0 || ncol(mat) == 0) {
      stop("Assay '", assay_name, "' in experiment '", exp_name, "' is empty (0 rows or 0 columns). Please verify your MAE.")
    }
    ...
    df_long <- tidyr::pivot_longer(df,
      cols = -featureID, names_to = "sampleID",
      values_to = "value"
    )
    ...
    if (!outcome.col %in% colnames(cd)) {
      # Survival-friendly fallback: if time+event exist, construct Y =
      # Surv(time, event)
...
.wrangle_data <- function(
  ...
  ft_wide <- tidyr::pivot_wider(ft, names_from = "sampleID", values_from = "value")
  ...
  list(feature_table = ft_wide, sample_metadata = sample_metadata, feature_metadata = feature_metadata)
}
...
.prepare_from_MAE <- function(
  ...
  if (!is.null(mae_valid)) {
    ...
    for (i in seq_along(experiment)) {
      ...
      if (!identical(f_train, f_valid)) {
        stop(
          "Feature sets differ between mae_train and mae_valid in experiment '",
  ...
  list(
    feature_table = feature_table, sample_metadata = sample_metadata, feature_metadata = feature_metadata,
    feature_table_valid = feature_table_valid, sample_metadata_valid = sample_metadata_valid
  )
}
...
.prepare_from_PCL <- function(
  ...
  if (length(missing_train) > 0) {
    missing_train_txt <- toString(missing_train)
    stop("PCL_train is missing required components: ", missing_train_txt)
  }
  ...
  if (!is.null(PCL_valid)) {
    ...
    if (length(missing_valid) > 0) {
      missing_valid_txt <- toString(missing_valid)
      stop("PCL_valid is missing required components: ", missing_valid_txt)
    }
...
.safe_family_name <- function(fam) {
  ...
  if (is.character(fam) && length(fam) > 0) {
    return(tolower(fam[1]))
  }
  NA_character_
}
...
.resolve_screening_args <- function(
  ...
  base_key <- if (is.null(base_screener)) {
    "all"
  } else {
    tolower(as.character(base_screener[1]))
  }
  via_base <- !(base_key %in% c(
    "all", "none", "noscreen", "noscreener", "off",
    "false", "0"
  ))

  enabled <- screen_flag || via_base
  if (!enabled && !is.null(screen_pct)) {
    warning("'screen_pct' was provided; enabling screening.", call. = FALSE)
    enabled <- TRUE
  }

  if (!enabled) {
    return(list(enabled = FALSE, screen_pct = NULL, via_base_screener = via_base))
  }

  if (is.null(screen_pct)) {
    stop("'", context, "': screening is enabled but 'screen_pct' is NULL. ",
      "Please provide the percentage of features to keep in (0,100].",
      call. = FALSE
    )
  }

  p_parsed <- type.convert(as.character(screen_pct[1]), as.is = TRUE)
  p <- if (is.numeric(p_parsed)) {
    as.numeric(p_parsed[[1]])
  } else {
    NA_real_
  }
  if (!is.finite(p)) {
  ...
  if (p > 0 && p <= 1) {
    warning("'screen_pct' was in (0,1]; interpreting as a proportion and converting to percent.",
      call. = FALSE
    )
    p <- p * 100
  }
        ...
        fit <- tryCatch(
          {
            ...
            glmnet::cv.glmnet(
              x = X_mat, y = y, family = "binomial", type.measure = "deviance",
              nfolds = inner_folds, weights = obsWeights
            )
          },
          error = function(e) NULL
        )
      ...
      fit <- tryCatch(
        {
          ...
          inner_folds <- min(5L, max(2L, nobs - 1L))
          glmnet::cv.glmnet(
            x = X_mat, y = y, family = "gaussian", type.measure = "mse",
            nfolds = inner_folds, weights = obsWeights
          )
        },
        error = function(e) NULL
...
.resolve_feature_filter_args <- function(
  ...
  if (!is.null(prevalence_pct) && !is.null(pct)) {
    stop("Provide only one of 'filter_pct' or legacy 'prevalence_pct', not both.",
      call. = FALSE
    )
  }

  if (!is.null(prevalence_pct)) {
    warning("'prevalence_pct' is deprecated; use 'filter_method = \"prevalence\"' and 'filter_pct'.",
      call. = FALSE
    )
    pct <- prevalence_pct
    if (is.null(method)) {
  ...
  if (is.null(pct)) {
    return(list(enabled = FALSE, filter_method = NULL, filter_pct = NULL, prevalence_pct = NULL))
  }

  p_parsed <- type.convert(as.character(pct[1]), as.is = TRUE)
  p <- if (is.numeric(p_parsed)) {
    as.numeric(p_parsed[[1]])
  } else {
    NA_real_
  ...
  if (p > 0 && p <= 1) {
    warning("'filter_pct' was in (0,1]; interpreting as a proportion and converting to percent.",
      call. = FALSE
    )
    p <- p * 100
  }
  ...
  method <- switch(method,
    ...
    stop("Unknown 'filter_method'. Supported values are 'prevalence' and 'variance'.",
      call. = FALSE
    )
  )

  list(enabled = TRUE, filter_method = method, filter_pct = p, prevalence_pct = if (identical(
    method,
    "prevalence"
  )) {
    p
...
.filter_features_by_method <- function(
  ...
  if (!isTRUE(resolved$enabled)) {
    return(list(
      ...
      feature_table_valid = feature_table_valid, filter_method = NULL, filter_pct = NULL,
      prevalence_pct = NULL, kept = rownames(feature_table)
    ))
  }
  ...
  if (!all(feature_ids %in% rownames(feature_metadata))) {
    stop("Could not align feature metadata to feature table rownames.", call. = FALSE)
  }
  ...
  .rank_layer_keep <- function(ids) {
    ...
    if (identical(resolved$filter_method, "prevalence")) {
      ...
      metrics <- tryCatch(as.data.frame(caret::nearZeroVar(t(x_sub), saveMetrics = TRUE)),
        error = function(e) NULL
      )
      if (is.null(metrics) || nrow(metrics) != nrow(x_sub)) {
        ...
        if (!"nzv" %in% names(metrics)) {
          metrics$nzv <- FALSE
        }
        if (!"zeroVar" %in% names(metrics)) {
          metrics$zeroVar <- FALSE
        }
        if (!"percentUnique" %in% names(metrics)) {
          metrics$percentUnique <- 0
        }

        ord <- order(metrics$nzv, metrics$zeroVar, -var_score, -metrics$percentUnique,
          rownames(x_sub),
          na.last = TRUE
        )
      }
  ...
  if (!is.null(feature_table_valid)) {
    if (!all(kept_ids %in% rownames(feature_table_valid))) {
      stop("Validation feature table is missing one or more retained training features after filtering.",
        call. = FALSE
      )
    }
  ...
  if (isTRUE(verbose)) {
    ...
    layer_msg <- paste0(names(total_by_layer), "=", as.integer(keep_by_layer[names(total_by_layer)]),
      "/", as.integer(total_by_layer),
      collapse = ", "
    )
    message(
      "Feature filter (", msg, ", top ", sprintf("%.2f", resolved$filter_pct),
      "% per layer): kept ", nrow(ft), "/", nrow(feature_table), " features. Layer breakdown: ",
      layer_msg, "."
    )
  }

  list(
    feature_table = ft, feature_metadata = fm, feature_table_valid = ft_valid,
    filter_method = resolved$filter_method, filter_pct = resolved$filter_pct,
    prevalence_pct = resolved$prevalence_pct, kept = kept_ids
  )
}
...
.ensure_survival_metadata <- function(df, context = "training") {
  ...
  stop(
    "Survival mode requires 'time' and 'event' columns in sample_metadata (",
    context, ")."
  )
}

compute_signed_univariate_importance <- function(
  feature_table, sample_metadata,
  feature_metadata, family
) {
  Y <- sample_metadata$Y
  ...
  beta_vec <- apply(feature_table, 1, function(x) {
    ...
    if (length(unique(x_use)) < 2 || length(y_use) < 3) {
      return(NA_real_)
    }

    fit <- tryCatch(if (identical(fam, "binomial")) {
      stats::glm(y_use ~ x_use, family = stats::binomial())
    } else {
      stats::lm(y_use ~ x_use)
    }, error = function(e) NULL)
    if (is.null(fit)) {
      return(NA_real_)
    }
