...
IL_multiclass <- function(
  ...
  screener_id <- if (isTRUE(screening$enabled)) {
    "glmnet"
  } else {
    "all"
  }
  screener_args <- if (isTRUE(screening$enabled)) {
    list(keep_prop = screening$screen_pct / 100, min_features = 1)
  } else {
    list()
  }
  ...
  for (i in seq_along(name_layers)) {
    if (isTRUE(verbose)) {
      message("Running multiclass base model for layer ", i, "...")
    }
    ...
    if (!is.null(feature_table_valid)) {
      dat_slice_valid_all <- as.data.frame(t(feature_table_valid[lay_features, ,
        drop = FALSE
      ]), check.names = FALSE)
      dat_slice_valid <- dat_slice_valid_all[, layer_screen$feature_names,
        drop = FALSE
      ]
  ...
  if (isTRUE(run_concat)) {
    if (isTRUE(verbose)) {
      message("Running multiclass concatenated model...")
    }
  ...
  if (!is.null(sample_metadata_valid) && !is.null(feature_table_valid)) {
    ...
    if (any(is.na(Y_test))) {
      warning("Validation set contains classes not observed in training; those samples are ignored for metric computation.",
        call. = FALSE
      )
    }
    ...
    if (isTRUE(run_stacked)) {
      prob_test$stacked <- stacked_valid_prob
    }
    if (isTRUE(run_concat)) {
  ...
  res <- list(
    model_fits = model_fits, X_train_layers = X_train_layers, X_train_concat = concat_train_matrix,
    Y_train = Y, prob.train = prob_train, class.train = class_train, yhat.train = class_train,
    metrics.train = metrics_train, class_levels = class_levels, base_learner = base_learner,
    base_learner_used = learner_id, meta_learner = meta_learner, meta_learner_used = meta_id,
    base_screener = base_screener, base_screener_used = if (isTRUE(screening$enabled)) "glmnet" else "none",
    screener_args_used = if (isTRUE(screening$enabled)) list(screen_pct = screening$screen_pct) else list(),
    screening_used = isTRUE(screening$enabled), screen_method = if (isTRUE(screening$enabled)) "glmnet" else NULL,
...
.multiclass_ovr_auc <- function(prob_mat, y_true) {
  ...
  auc_by_class <- vapply(cls, function(cl) {
    ...
    if (sum(y_bin == 1L) == 0L || sum(y_bin == 0L) == 0L) {
      return(NA_real_)
    }
    ...
    pred_obj <- tryCatch(ROCR::prediction(predictions = score, labels = y_bin),
...
.map_multiclass_learner <- function(base_learner) {
  ...
  alias_map <- list(
    glmnet = c(
      ...
      "slrandomforest", "multiclassrandomforest", "randomforest",
      "rf"
    ), ranger = c("slranger", "multiclassranger", "ranger"), xgboost = c(
      "multiclassxgboost",
      "xgboost", "xgb", "slxgboost"
  ...
  warning("base_learner '", base_learner, "' is not natively supported for multiclass in this backend; using glmnet multinomial.",
    call. = FALSE
  )
  "glmnet"
}
...
.map_multiclass_meta_learner <- function(meta_learner) {
  ...
  alias_map <- list(
    glmnet = c(
      "slmulticlasslogloss", "slglmnet", "multiclassglmnet",
      "glmnet"
    ), randomforest = c("multiclassrandomforest", "randomforest", "rf"),
    ranger = c("multiclassranger", "ranger"), xgboost = c(
      "multiclassxgboost",
      "xgboost", "xgb"
    ), mbart = c("multiclassmbart", "mbart", "bart"), multinom = c(
      "multiclassmultinom",
      "multinom", "nnetmultinom"
    )
  )
  for (nm in names(alias_map)) {
    if (key %in% alias_map[[nm]]) {
  ...
  warning("meta_learner '", meta_learner, "' is not natively supported for multiclass stacking in this backend; using glmnet multinomial.",
    call. = FALSE
  )
  "glmnet"
}
...
.map_multiclass_screener <- function(base_screener) {
  ...
  alias_map <- list(
    ...
    anova = c("anova", "fstat", "screenanova", "slanova"), glmnet = c(
      "glmnet",
      "slglmnet", "screenglmnet"
    ), randomforest = c(
      "randomforest", "rf", "slrandomforest",
      "screenrandomforest"
  ...
  warning("base_screener '", base_screener, "' is not natively supported for multiclass screening in this backend; using all features.",
    call. = FALSE
  )
  "all"
}
...
.select_multiclass_screened_features <- function(scores, feature_names, screener_args = list()) {
  ...
  .scalar_num <- function(x) {
    ...
    parsed <- type.convert(as.character(x[[1]]), as.is = TRUE)
...
.fit_multiclass_screener <- function(X, y, screener_id = "all", seed = 1234, screener_args = list()) {
  ...
  if (identical(screener_id, "all") || ncol(X_df) <= 1L) {
    return(list(feature_names = feature_names, scores = stats::setNames(rep(
      1,
      length(feature_names)
    ), feature_names), screener_id = "all"))
  }
  ...
  scores <- tryCatch(
    {
      if (identical(screener_id, "anova")) {
        ...
        fit <- glmnet::cv.glmnet(
          x = as.matrix(X_df), y = y_fac, family = "multinomial",
          type.measure = "deviance", nfolds = inner_folds
        )
        ...
        fit <- ranger::ranger(
          x = X_df, y = y_fac, probability = TRUE, num.trees = num_trees,
          importance = "impurity", write.forest = FALSE
        )
        ...
        dtrain <- xgboost::xgb.DMatrix(data = X_mat, label = as.integer(y_fac) -
          1L)
        fit <- xgboost::xgb.train(
          data = dtrain, nrounds = nrounds, params = list(
            objective = "multi:softprob",
            num_class = as.integer(nlevels(y_fac)), eta = eta, max_depth = max_depth,
    ...
    error = function(e) {
      warning("Screener '", screener_id, "' failed (", conditionMessage(e), "); using all features for this fit.",
        call. = FALSE
      )
...
.reshape_multiclass_probs <- function(pred, n_obs, n_classes, learner_id) {
  if (is.null(dim(pred))) {
    if (length(pred) != n_obs * n_classes) {
      stop("Learner '", learner_id, "' returned ", length(pred), " predictions; expected ",
        n_obs * n_classes, " for multiclass probabilities.",
        call. = FALSE
      )
    }
  ...
  if (nrow(out) != n_obs || ncol(out) != n_classes) {
    stop("Learner '", learner_id, "' returned a probability matrix with shape ",
...
.fit_oof_multiclass <- function(
  ...
  for (f in sort(unique(fold_id))) {
    ...
    if (length(unique(as.character(y_train))) < 2L) {
      fill <- matrix(rep(prior, each = length(valid_idx)),
        nrow = length(valid_idx),
        byrow = FALSE
      )
...
.fit_multiclass_model_impl <- function(
  ...
  if (identical(learner_id, "glmnet")) {
    ...
    defaults <- list(
      x = X_df, y = y_fac, probability = TRUE, num.trees = 500,
      write.forest = TRUE
    )
    ...
    defaults <- list(data = dtrain, nrounds = 200L, params = list(
      objective = "multi:softprob",
      num_class = as.integer(nlevels(y_fac)), eta = 0.05, max_depth = 6L, subsample = 0.8,
    ...
    defaults <- list(
      x.train = as.matrix(X_df), y.train = y_num, x.test = matrix(0,
        nrow = 0L, ncol = 0L
      ), type = "pbart", ntype = ntype_pbart, rm.const = FALSE,
      ntree = 50L, ndpost = 200L, nskip = 100L, keepevery = 1L, printevery = 1000L
    )
  ...
  list(
    learner_id = learner_id, model = fit, class_levels = model_class_levels,
    feature_names = colnames(X_df)
  )
}
...
.align_newdata_to_training_features <- function(X_df, fit_obj) {
  ...
  if (is.null(colnames(X_df))) {
    if (ncol(X_df) != length(train_features)) {
      stop("Prediction data has ", ncol(X_df), " columns but learner '", fit_obj$learner_id,
        "' expects ", length(train_features), ".",
        call. = FALSE
      )
    }
  ...
  if (length(missing_cols) > 0L) {
    if (ncol(X_df) == length(train_features)) {
      warning("Could not align prediction columns by name for learner '", fit_obj$learner_id,
    ...
    stop("Prediction data for learner '", fit_obj$learner_id, "' is missing ",
...
.predict_multiclass_model_impl <- function(fit_obj, newX, class_levels = NULL, eps = 1e-15) {
  ...
  if (identical(fit_obj$learner_id, "glmnet")) {
    raw <- stats::predict(fit_obj$model,
      newx = as.matrix(X_df), s = "lambda.min",
      type = "response"
    )
    ...
    n_model_classes <- if (!is.null(fit_obj$class_levels)) {
      length(fit_obj$class_levels)
    } else {
      length(class_levels)
    }
  ...
  if (is.null(colnames(prob))) {
    if (!is.null(fit_obj$class_levels) && ncol(prob) == length(fit_obj$class_levels)) {
      ...
      warning("Could not infer all multiclass probability column names; using positional alignment.",
        call. = FALSE
      )
    }
  }
...
.sanitize_prob_matrix <- function(prob_mat, class_levels, eps = 1e-15) {
  ...
  if (is.null(colnames(p))) {
    if (ncol(p) != length(class_levels)) {
      stop("Probability output has no class names and incompatible column count.",
        call. = FALSE
      )
    }
...
.compute_multiclass_univariate_importance <- function(
  ...
  by_class <- t(vapply(rownames(feature_table), function(fid) {
    ...
    vapply(class_levels, function(cl) {
      in_cl <- y == cl
      out_cl <- y != cl
      if (sum(in_cl, na.rm = TRUE) < 2L || sum(out_cl, na.rm = TRUE) < 2L) {
        return(NA_real_)
      }
  ...
  by_layer_class <- lapply(
    split(rownames(by_class), feature_metadata$featureType),
    function(ids) {
  ...
  list(by_class = by_class, by_layer_class = by_layer_class, global = global)
}

predict_multiclass.learner <- function(
  object, feature_table_valid = NULL, sample_metadata_valid = NULL,
  feature_metadata = NULL, eps = 1e-15, ...
) {
  fit <- object
  ...
  res <- list(prob.test = out_prob, class.test = out_class, yhat.test = out_class)
