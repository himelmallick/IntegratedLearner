# This is a basic utility function to plot the top 20 feasture importance
...
VarImp.learner <- function(fit, num.var = 20, layer.names = NULL) {
  ...
  if (fit$base_learner == "SL.BART") {
    ...
    if (!all(layer.names %in% names(fit$model_fits$model_layers))) {
      invalid_layers <- layer.names[!(layer.names %in% names(fit$model_fits$model_layers))]
      invalid_layers_txt <- toString(invalid_layers)
      stop("Invalid layer name(s): ", invalid_layers_txt, ".", call. = FALSE)
    }
    ...
    for (i in seq_along(layer.names)) {
      qq <- bartMachine::investigate_var_importance(fit$model_fits$model_layers[[layer.names[i]]],
        plot = FALSE
      )
      ...
      n_keep <- min(as.integer(num.var), nrow(VIMP_layer))
  ...
  p <- ggplot2::ggplot(VIMP, ggplot2::aes(stats::reorder(ID, -mean), mean, fill = type)) +
    ...
    ggplot2::geom_errorbar(ggplot2::aes(ymin = ifelse(mean -
      sd > 0, mean - sd, 0), ymax = mean + sd), width = 0.2, position = ggplot2::position_dodge(0.9)) +
    ggplot2::theme_bw() +
    ggplot2::coord_flip() +
    omicsEye_theme() +
...
omicsEye_theme <- function() {
  ...
  ggplot2::theme_bw() + ggplot2::theme(
    axis.text.x = ggplot2::element_text(
      size = 8,
      vjust = 1, hjust = hjust, angle = angle
    ), axis.text.y = ggplot2::element_text(
      size = 8,
      hjust = 1
    ), axis.title = ggplot2::element_text(size = 10), plot.title = ggplot2::element_text(size = 10),
    plot.subtitle = ggplot2::element_text(size = 8), legend.title = ggplot2::element_text(
      size = 6,
      face = "bold"
    ), legend.text = ggplot2::element_text(size = 7), axis.line = ggplot2::element_line(
      colour = "black",
      linewidth = 0.25
